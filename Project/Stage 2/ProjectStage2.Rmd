...
```{r}
hist(variables$Years_of_Education, breaks = "fd")
...
Reg <- glm(Action_Taken ~ Debt_to_Income_Total + Race + Self_Employed + Marital_Status + Years_of_Education , data = variables ,family = "binomial")
...
my_roc <- roc(variables$Action_Taken ~ prob, plot = TRUE)
my_auc <- auc(my_roc)
...
```{r}
y <- variables$Action_Taken
x <- variables[-c(8)]

cv.housingreg <- cv.gamlr(x, y, 
	free=1,family="binomial", verb=TRUE, standardize=TRUE, nfold = 10)
```
